...
function Install-TemurinJdk {
  ...
  $tmpFile = $null
  $downloadUrl = $null
  # Prefer querying the assets API to obtain a direct package link and checksum
  # Try multiple architecture and OS name variants to increase chances of matching the API
          ...
          if ($null -ne $entry.binaries) {
            foreach ($b in $entry.binaries) {
              if ($null -ne $b.package -and $b.package.link) {
                # Use the package.link as the reliable download URL
  ...
  # Fallback: try the older binary endpoint pattern across variants if assets API didn't yield a package link
  if (-not $downloadOk) {
    foreach ($trialArch in $archVariants) {
      foreach ($trialOs in $osVariants) {
