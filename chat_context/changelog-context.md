...
- (2025-11-28, 09:50) Context cleanup completed: centralized `.summary.md` into `chat_context/.summaries/`, purged stray top-level summaries, and offloaded `ATTACHMENTS.md` + `knowledge-compartmentalization.md` to `archives/`. Added PowerShell shell integration (`LereTools.psm1` with `hc`, `offload`, `purgesummaries`, `ctxsum`) and fixed module export/alias issues. Updated `package-release.yml` to use `hashFiles()` check and collect Gradle jars from `*/build/libs/*.jar`. Health checks updated (exclude `offloads/`, case-accept PRIVACY/ATTACHMENTS, ProjectConfig top-level cap = 12). Final top-level files: 10 (within 12). See (Session 2025-11-28 09:50). [changelog-entry:2025-11-28 09:50]
...
- (2025-11-25, 17:59) Manual setup of context memory scaffolding by user. See (Session 2025-11-25 17:59).
 ...
 - (2025-11-27, 23:58) Created new plugin scaffold `plugins/lere_multiplayer` (Gradle build, main class, plugin.yml, config, README). See (Session 2025-11-27 23:58). [changelog-entry:2025-11-27 23:58]
 - (2025-11-28, 00:10) Added privacy & anonymization policy to `chat_context/privacy.md` and updated `general-chat-context.md` to reference it. See (Session 2025-11-28 00:10). [changelog-entry:2025-11-28 00:10]
